...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operand_evaluator = lambda o: scheme_eval(o, env)
        evaluated_operands = operands.map(operand_evaluator)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('inconsistent lengths')
        else:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('incorrect number of arguments provided')
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # Binds TARGET to VALUE in environment given by ENV
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    value = True
    while expressions and is_true_primitive(value):
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    value = False
    while expressions and is_false_primitive(value):
        value = scheme_eval(expressions.first, env)
        expressions = expressions.rest
    return value
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            sub_expressions = clause.rest
            if sub_expressions:
                return eval_all(sub_expressions, env)
            else:
                return test
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        b = bindings.first
        validate_form(b, 2, 2)
        names = Pair(b.first, names)
        values = Pair(scheme_eval(b.rest.first, env), values)
